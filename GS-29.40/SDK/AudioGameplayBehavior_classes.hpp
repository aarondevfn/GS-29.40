...
{
public:
	uint8                                         Pad_A698[0x8];                                     // 0x00A0(0x0008)(Fixing Size After Last Property [ Dumper-7 ])
	...
	uint8                                         Pad_A699[0x6];                                     // 0x00AA(0x0006)(Fixing Size After Last Property [ Dumper-7 ])
	...
	uint8                                         Pad_A69A[0x10];                                    // 0x00E8(0x0010)(Fixing Struct Size After Last Property [ Dumper-7 ])
...
public:
	...
	struct FActiveVoice PlaySound(class USoundBase* Sound, float InFadeInTime, float InTargetVolume, float InStartTime, bool bDisableAttenuation, EAudioFaderCurve InFadeCurve, struct FVector& InRelativeLocation, struct FRotator& InRelativeRotation);
	void SetPlayState(EAudioGameplayBehaviorPlayState NewState);
	void Start();
	void Stop();
	void StopAllPlayingVoices(float InFadeOutTime);
	void StopById(const struct FPlayingId& ID, float InFadeOutTime, EAudioFaderCurve InFadeCurve);
	void StopComponent(class UAudioComponent* Comp, float InFadeOutTime, EAudioFaderCurve InFadeCurve);
	void StopSound(class USoundBase* Sound, float InFadeOutTime, EAudioFaderCurve InFadeCurve);
	...
	bool GetBoolParamValue(class FName ParamName) const;
	float GetFloatParamValue(class FName ParamName) const;
	class UAudioComponentGroup* GetSoundGroup() const;
	class FString GetStringParamValue(class FName ParamName) const;
